...
const MobileMenuDropDown = (props) => {
  ...
  const [user_id, setUser_id] = useState(
    props.applicationState.appReducer.user_id
  );
    ...
    const getUserData = async () => {
      try {
        const allUsersData = await axios.get(
          `http://localhost:7777/userdata/?user_id=${user_id}`
        );
      ...
      <div class="block bg-gray-800">
        <div class="mt-12 mb-6">
          <Link
            ...
            className={
              activeMenu === "Home"
                ? "block bg-gray-900 text-white rounded-md text-md font-medium px-3 py-3 w-full"
                : "block text-gray-300 text-md rounded-md font-medium px-3 py-3 w-full"
            }
        ...
        <div class="border-t border-gray-700">
          {props.showLogin ? (
            <MobileMenuDropdownSignedin
              show={props.show}
              setShowLogin={props.setShowLogin}
            />
          ) : (
            <MobileMenuDropdownLogin
              show={props.show}
