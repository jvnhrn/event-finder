...
function ExploreCard(props) {
  ...
  const [openSpotsColor, setOpenSpotsColor] = useState(
    "text-gray-400 px-6 font-semibold tracking-tighter"
  );
  ...
  return (
    <div class="px-6 py-6 flex-wrap flex">
      <div class="max-w-sm rounded-3xl overflow-hidden shadow-lg mr-10 mb-10">
        ...
        <div className="px-8 py-4">
          <div className="font-bold text-gray-600 my-4 text-left tracking-tighter">
            STARTS:{" "}
            {(new Date(props.event_start_date) + new Date().getTimezoneOffset())
              .toString()
              .substr(0, 21)}
          </div>
          <div className="font-bold text-gray-600 my-4 text-left tracking-tighter">
            ENDS:{" "}
            {(new Date(props.event_end_date) + new Date().getTimezoneOffset())
              .toString()
              .substr(0, 21)}
          </div>
          ...
          <p
            ...
            id="address"
          >
            {props.event_country}, {props.event_city}, {props.event_postalcode},{" "}
            {props.event_address}
          </p>
          ...
          <ul className="py-4 text-left tracking-tighter">
            ...
            <li>{props.event_host_phone}</li>
            <li>{props.event_host_email}</li>
          </ul>
        </div>
        ...
        <div class="pb-8 grid grid-cols-4 gap-4">
          <div>
            <span
              id="yoga"
              className="text-sm text-gray-400 px-5 font-semibold tracking-tighter"
            >
              {props.event_category}
            </span>
          </div>
          <div>
            <span
              id="pricing"
              className="text-sm text-gray-400 px-4 font-semibold tracking-tighter"
            >
              € {props.event_price}
            </span>
          </div>
          <div>
            <span
              id="location"
              className="text-sm text-gray-400 px-2 font-semibold tracking-tighter"
            >
              {props.event_location}
            </span>
          </div>
          <div>
            <span id="leftspots" className={openSpotsColor}>
              {props.open_spots}
            </span>
          </div>
        </div>
