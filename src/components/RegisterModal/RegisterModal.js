  ...
  const registerUser = async () => {
    try {
      if (user_password == user_password_repeat) {
        ...
        const sendUserCredentials = await axios.post(
          `http://localhost:7777/registeruser`,
          newUserData
        );
      ...
      <div class="bg-white max-w-lg mx-auto p-8 md:p-12 my-10 rounded-lg shadow-2xl">
        ...
        <section class="mt-10">
          ...
          <div class="mb-6 pt-3 rounded bg-gray-200">
            ...
            <input
              onChange={(e) => set_user_birthdate(e.target.value)}
              type="date"
              id="password3"
              class="bg-gray-200 rounded w-full text-gray-700 focus:outline-none border-b-4 border-gray-300 focus:border-blue-600 transition duration-500 px-3 pb-3"
            />
          ...
          <div class="mb-6 pt-3 rounded bg-gray-200">
            ...
            <input
              onChange={(e) => set_user_password_repeat(e.target.value)}
              type="password"
              id="password1"
              class="bg-gray-200 rounded w-full text-gray-700 focus:outline-none border-b-4 border-gray-300 focus:border-blue-600 transition duration-500 px-3 pb-3"
            />
          </div>
          <div class="mb-6 rounded bg-red-200">{wrongPassword}</div>
          {!!userRegistered ? (
            <div class="mb-6 rounded bg-red-200">
              {" "}
              User (username, email or phone number) already exists
            </div>
          ) : (
            <div></div>
          )}
          <div class="flex justify-center">
            <button
              ...
              class="bg-blue-600 hover:bg-blue-800 text-white font-bold uppercase py-2 px-4 rounded shadow-lg hover:shadow-xl outline-none focus:outline-none transition duration-200" /* type="submit" */
            >
              Sign Up
            </button>
          </div>{" "}
      ...
      <div class="max-w-lg mx-auto text-center mt-12 mb-6">
        <button class="text-gray-100">
          ...
          <a
            ...
            class="font-bold px-1 py-1 rounded hover:text-purple-600 hover:underline hover:bg-gray-100"
          >
            Login
          </a>
        </button>
